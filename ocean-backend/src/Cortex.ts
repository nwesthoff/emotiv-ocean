...
export default class Cortex {
  ...
  queryHeadsetId() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == QUERY_HEADSET_ID) {
            // console.log(data)
            // console.log(JSON.parse(data.toString())['result'].length)
            if (JSON.parse(data.toString())["result"].length > 0) {
              let headsetId = JSON.parse(data.toString())["result"][0]["id"];
              resolve(headsetId);
            } else {
              console.log(
  ...
  requestAccess() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == REQUEST_ACCESS_ID) {
            resolve(data);
          }
        } catch (error) {}
      });
  ...
  authorize() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == AUTHORIZE_ID) {
            let cortexToken = JSON.parse(data.toString())["result"][
              "cortexToken"
            ];
            resolve(cortexToken);
          }
  ...
  controlDevice(headsetId) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == CONTROL_DEVICE_ID) {
            resolve(data);
          }
        } catch (error) {}
      });
  ...
  createSession(authToken: string, headsetId) {
    let socket = this.socket;
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        // console.log(data)
        try {
          if (JSON.parse(data.toString())["id"] == CREATE_SESSION_ID) {
            let sessionId = JSON.parse(data.toString())["result"]["id"];
            resolve(sessionId);
          }
        } catch (error) {}
      });
  ...
  startRecord(authToken: string, sessionId: string, recordName: string) {
    let socket = this.socket;
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == CREATE_RECORD_REQUEST_ID) {
            console.log(
  ...
  injectMarkerRequest(authToken, sessionId, label, value, port, time) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == INJECT_MARKER_REQUEST_ID) {
            console.log(
  ...
  stopRecord(authToken: string, sessionId: string, recordName: string) {
    let socket = this.socket;
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == STOP_RECORD_REQUEST_ID) {
  ...
  subRequest(stream, authToken, sessionId) {
    ...
    // socket.on("message", (data) => {
    //   try {
    //     // if(JSON.parse(data.toString())['id']==SUB_REQUEST_ID){
  ...
  mentalCommandActiveActionRequest(authToken, sessionId, profile, action) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (
            JSON.parse(data.toString())["id"] == MENTAL_COMMAND_ACTIVE_ACTION_ID
          ) {
            console.log(
   ...
   */
  async sub(streams, handleResult: (data) => void) {
    this.socket.on("open", async () => {
      await this.checkGrantAccessAndQuerySessionInfo();
      this.subRequest(streams, this.authToken, this.sessionId);
      this.socket.on("message", (data) => {
        handleResult(JSON.parse(data.toString()));
      });
    });
  }
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        ...
        try {
          ...
          if (JSON.parse(data.toString())["id"] == SETUP_PROFILE_ID) {
            if (JSON.parse(data.toString())["result"]["action"] == status) {
              console.log(
                "SETUP PROFILE -------------------------------------"
              );
              console.log(data);
  ...
  queryProfileRequest(authToken) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          if (JSON.parse(data.toString())["id"] == QUERY_PROFILE_ID) {
            // console.log(data)
            resolve(data);
          }
        } catch (error) {}
  ...
  trainRequest(authToken, sessionId, action, status) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        ...
        try {
          if (JSON.parse(data.toString())["id"] == TRAINING_ID) {
            console.log(data);
          }
        } catch (error) {}
        ...
        if (status == "start") {
          try {
            if (JSON.parse(data.toString())["sys"][1] == "MC_Succeeded") {
              console.log(
        ...
        if (status == "accept") {
          try {
            if (JSON.parse(data.toString())["sys"][1] == "MC_Completed") {
              console.log(
  ...
  live(profileName: string, handleResult: (data) => void) {
    ...
    this.socket.on("open", async () => {
      ...
      let loadProfileResult;
      let status = "load";
      console.log(this.authToken, this.headsetId);
      await this.setupProfile(
        this.authToken,
        this.headsetId,
        profileName,
